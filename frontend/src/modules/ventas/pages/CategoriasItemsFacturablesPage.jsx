...
const CategoriasItemsFacturablesPage = () => {
  ...
  const loadItems = async () => {
    ...
    setError('');
    try {
      setItems(await ventasService.getCategoriasItemsFacturables());
    } catch (loadError) {
  ...
  useEffect(() => {
    let mounted = true;
    ventasService.getCategoriasItemsFacturables()
      .then((data) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (selected?.id) {
        await ventasService.updateCategoriaItemFacturable(selected.id, payload);
        setMessage('Categoria actualizada.');
      } else {
        await ventasService.createCategoriaItemFacturable(payload);
  ...
  const handleToggle = async (item) => {
    ...
    setError('');
    try {
      await ventasService.updateCategoriaItemFacturable(item.id, buildPayload({ ...item, activo: !item.activo }));
      setMessage(item.activo ? 'Categoria desactivada.' : 'Categoria activada.');
      await loadItems();
    } catch (toggleError) {
  ...
  return (
    <div className="page-container ventas-page">
      <div className="page-header">
        <div>
          <h1>Categorias de items facturables</h1>
          <p className="page-subtitle">Agrupadores opcionales para el catalogo de items facturables.</p>
        </div>
      ...
      <SectionCard title={selected ? 'Editar categoria' : 'Nueva categoria'}>
      ...
      <SectionCard title="Categorias existentes">
        ...
        {loading ? <LoadingSpinner /> : (
          <div className="table-wrapper">
            ...
            {!filteredItems.length && <p className="empty-state">No hay categorias para los filtros seleccionados.</p>}
          </div>
        )}
      </SectionCard>
    </div>
...
export default CategoriasItemsFacturablesPage;
