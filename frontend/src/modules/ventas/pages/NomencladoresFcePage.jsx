...
const NomencladoresFcePage = () => {
  ...
  const loadItems = async () => {
    ...
    setError('');
    try {
      setItems(await ventasService.getNomencladores());
    } catch (loadError) {
  ...
  useEffect(() => {
    let mounted = true;
    ventasService.getNomencladores()
      .then((data) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (selected?.id) {
        await ventasService.updateNomenclador(selected.id, payload);
        setMessage('Nomenclador actualizado.');
      } else {
        await ventasService.createNomenclador(payload);
        setMessage('Nomenclador creado.');
      }
      resetForm();
      await loadItems();
    } catch (saveError) {
  ...
  const handleToggle = async (item) => {
    ...
    setError('');
    try {
      await ventasService.updateNomenclador(item.id, buildPayload({ ...item, activo: !item.activo }));
      setMessage(item.activo ? 'Nomenclador desactivado.' : 'Nomenclador activado.');
      await loadItems();
    } catch (toggleError) {
  ...
  return (
    <div className="page-container ventas-page">
      <div className="page-header">
        <div>
          <h1>Nomencladores FCE</h1>
          <p className="page-subtitle">Catalogo para futuras Facturas de Credito Electronica MiPyME con nomenclador.</p>
        </div>
      ...
      <SectionCard title={selected ? 'Editar nomenclador' : 'Nuevo nomenclador'}>
      ...
      <SectionCard title="Nomencladores existentes">
        ...
        {loading ? <LoadingSpinner /> : (
          <div className="table-wrapper">
            ...
            {!filteredItems.length && <p className="empty-state">No hay nomencladores para los filtros seleccionados.</p>}
          </div>
        )}
      </SectionCard>
    </div>
...
export default NomencladoresFcePage;
